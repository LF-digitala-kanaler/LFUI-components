import{a as p,D as te,R as h,s as N,u as V,h as J}from"./utils-de29bc01.js";import"./hooks.module-446c997e.js";import"./jsxRuntime.module-779d8325.js";const G=new WeakSet;function oe(t,n=t.dataset){const o=ne(n),e=t.querySelector(".select-options"),B=t.querySelector(".select-label"),c=Math.random().toString(36).substr(2,9);if(!B||!e||e.disabled||G.has(t))return;let f=!1;const Q=[...e.querySelectorAll("[selected]")];let u=e.querySelector('option[value=""][disabled]');const y=Q.filter(a=>!H(a));A(e,y.length);const C=new WeakMap,$=new WeakMap,R=new h,z=new h,M=p("button",{type:"button",title:B.textContent,class:"select-toggle dropdown-toggle","data-bs-toggle":"dropdown","aria-expanded":"false","aria-controls":`list-${c}`},[p("span",{"data-count":y.length||"",class:"select-status",role:"status",ref:z},y.length?W(y,o.smallPattern):u?k(u.label,o.smallPattern):"")]);function q(){return[e.multiple&&o.allLabel?p("button",{ref:R,type:"button",class:"select-option multiple dropdown-item",onclick(a){const l=!O(e);for(const r of e.options)r.disabled||(r.selected=l);e.dispatchEvent(new window.Event("change",{bubbles:!0})),a.preventDefault()}},o.allLabel):null,...[...e.children].map(function a(l,r){if(l.nodeName==="OPTGROUP"){const g=k(l.label,o.smallPattern);let s=[...l.children].map(a);const d=new h,v=new h;let i=!0;if(o.groupToggle){const L=p("fieldset",{"data-bs-target":`#list-${c}`,id:`group-${r}-list-${c}`,class:"select-list dropdown-menu","aria-labelledby":`group-${r}-trigger-${c}`},s);for(const D of l.children)$.set(D,v);s=[L]}return p(o.groupToggle?"div":"fieldset",{ref:d,class:`select-group ${o.groupToggle?"show":""}`},[o.groupToggle?p("button",{ref:v,type:"button",id:`group-${r}-trigger-${c}`,class:`select-option ${e.multiple?"multiple":""} toggle dropdown-item`,"aria-expanded":"true","aria-controls":`group-${r}-list-${c}`,onclick(){i=!i,this.setAttribute("aria-expanded",i?"true":"false"),d.current.classList.toggle("show",i),f=!0;for(const L of l.children)C.get(L).current.classList.toggle("disabled",!i);X.update()}},g):p("legend",{class:"select-legend"},g),...s])}if(H(l))return null;const{label:m,disabled:b,selected:E}=l,S=new h;return C.set(l,S),p("button",{disabled:b,ref:S,type:"button",class:`select-option dropdown-item ${e.multiple?"multiple":""} ${E?"selected":""}`,onclick(g){l.selected=e.multiple?!l.selected:!0,e.dispatchEvent(new window.Event("change",{bubbles:!0}))}},k(m,o.smallPattern,b?"":void 0))})]}const T=p("div",{id:`list-${c}`,"aria-labelledby":`label-${c}`,class:"select-list dropdown-menu",onclick(){f=f||e.multiple}},q());t.addEventListener("hide.bs.dropdown",function(a){f&&(f=!1,a.preventDefault())}),e.addEventListener("change",function(){var g;(g=R.current)==null||g.classList.toggle("selected",O(e));const a=new Map;for(const s of e.options){if(s.disabled)continue;const d=C.get(s);if(d==null||d.current.classList.toggle("selected",s.selected),$.has(s)){const v=$.get(s);let i=a.get(v);s.selected?i?i==="unselected"&&(i="indeterminate"):i="selected":i==="selected"?i="indeterminate":i||(i="unselected"),a.set(v,i)}}for(const[s,d]of a)s.current.classList.toggle("selected",d==="selected"),s.current.classList.toggle("indeterminate",d==="indeterminate");const l=[...e.selectedOptions].filter(s=>!H(s)),{current:r}=z;A(e,l.length),r.dataset.count=l.length||"";const m=l.length?l:u?[u]:[],[b,...E]=r.childNodes,S=m.length?W(m,o.smallPattern):[""];for(const s of E)s.remove();b.replaceWith(...S)}),t.classList.add("initialized"),B.id=`label-${c}`,e.after(M,T);const X=new te(M);G.add(t);const ee=(a,l)=>{for(const r of a)if(r.type==="childList"||r.type==="carachterData"){const m=document.createDocumentFragment();m.append(...q().filter(b=>b)),T.innerHTML="",T.append(m),u=u=e.querySelector('option[value=""][disabled]'),e.dispatchEvent(new window.Event("change"))}};new window.MutationObserver(ee).observe(e,{attributes:!1,childList:!0,subtree:!0,characterData:!0})}function ne(t){let{smallPattern:n,allLabel:o,groupToggle:e}=t;return n&&!(n instanceof RegExp)&&(n=new RegExp(n)),typeof e<"u"&&(e=e!==!1),{smallPattern:n,allLabel:o,groupToggle:e}}function A(t,n){const o=t.closest(".form-group");o&&o.classList.toggle("has-valid",n)}function H(t){const{value:n,disabled:o,hidden:e}=t;return!n&&o&&e}function O(t){return t.selectedOptions.length===t.options.length}function W(t,n){return t.reduce(function(o,e){return o.length&&o.push(", "),o.push(...k(e.label,n)),o},[])}function k(t,n,o="text-muted"){const e=t.match(n);return e?[t.replace(n,""),p("small",{class:o},e[1]||e[0])]:[t]}const le=`<div class="row">
  <div class="col-md-6">
    ...
    <form class="form-group">
      <div class="select inside">
        ...
        <select class="select-options" name="timezone" id="select-label-inside">
          <option value="" disabled hidden>Select asdasd</option>
          <option>Stockholm</option>
          <option>Göteborg</option>
          <option>Malmö</option>
          <option>Umeå</option>
...
//# sourceMappingURL=select.stories-d49bd69e.js.map
