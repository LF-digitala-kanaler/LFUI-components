...
const lineChartExample = () => {
  ...
  const gradientFill = ctx.createLinearGradient(0, 800, 0, 0)
  ...
  document.getElementById('js-chartLegends').insertAdjacentHTML('beforeend', window.lineChart.generateLegend())
  let init = false
  window.updateDataset = function (event, datasetIndex) {
    const index = parseInt(datasetIndex)
    const ci = event.view.lineChart
    ...
    document.getElementById('js-chartLegends').innerHTML = window.lineChart.generateLegend()
    ci.update()
  }
  ...
  document.getElementById('addDataset').addEventListener('click', function () {
    ...
    config.data.datasets.forEach((item, i) => {
      item.borderColor = color[i]
    })
