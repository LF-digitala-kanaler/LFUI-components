...
<Note>
	__NOTE__ Do not add `.first` & `.last`-classes to the `.table-varied-variant`, as it breaks its
	styling.
</Note>
...
<Note>
	__Note:__ Since the collapse button does not have the `.btn-class`, make sure you add the
	`role="button"`-property to the element (and of course, as usual, the relevant aria-attributes!).
</Note>
