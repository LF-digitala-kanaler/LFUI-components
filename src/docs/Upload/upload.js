const uploadExample = () => {
  ...
  const placeholder = element.querySelector('.upload-placeholder')
  const remove = element.querySelector('.upload-remove')
  ...
  $('#upload-test-btn').on('click', function () {
    ...
    const loaderTest = setInterval(function () {
      ...
      if (timeleft <= 0) { clearInterval(loaderTest) }
      if (timeleft === 0) { $('#upload2').parent().parent().addClass('has-valid') }
