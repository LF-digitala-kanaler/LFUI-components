...
export class Select {
  constructor(element, opts = element.dataset) {
    ...
    this.initiallyValid = this.initiallySelected.filter(
    ...
    const callback = (mutationList) => {
      for (const mutation of mutationList) {
        if (
          ...
          this.placeholder = this.select.querySelector(
            'option[value=""][disabled]'
          );
          this.selectableItems = [
            ...this.list.querySelectorAll(".select-option"),
          ];
          this.searchableItems = this.selectableItems.filter(notDisabled);
  ...
  moveFocusTo(position) {
    ...
    if (position === "selected") {
      const selectedItem =
        list.querySelector(".select-option--selected") ||
    ...
    setTimeout(() => {
      list.setAttribute(
        ...
        list.querySelector(".select-option--focus").id
      );
  ...
  onChangeHandler() {
    ...
    for (const [button, state] of states) {
      ...
      button.current.classList.toggle(
        "select-option--indeterminate",
        state === "indeterminate"
      );
    }

    const selected = [...this.select.selectedOptions].filter(
  ...
  createOptions() {
    ...
    return [
      ...
      ...[...select.children].map((child, index) =>
        this.createListItem(child, index)
      ),
    ];
  }
  ...
  selectOption(target) {
    ...
    if (target._selectElement) {
      target._selectElement.selected = this.multiselectable
        ? !target._selectElement.selected
        : true;
    }
